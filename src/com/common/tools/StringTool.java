...
public class StringTool {
	 ...
	 * 将一个数组用分隔符连接起来。
	 * @param items - 数组
	 * @param split - 分隔符，如果是null或""，则无分隔符。
	...
	/**
	 * 上传文件时，比较上传用户与系统中存储的字段是否一致
	 * @param sysname 系统中保存的用户名
	 * @param uploadname 上传文件中的用户名
	 * @return    相同放回true
	 */
	public static boolean compareName(String sysname, String uploadname){
		boolean result=false;
		//如果不相等，则比较前两位,正常情况下姓名是>=2位的
		if(!sysname.equals(uploadname)){
			String s = sysname.replace(" ", "").substring(0, 2);
			String u = uploadname.replace(" ", "").substring(0, 2);
			if(s.equals(u)){
				result = true;
			}	
		}else{
	...
	public static void main(String[] args) {
		System.out.println(StringTool.escapeSql("d'd"));
	}
}
