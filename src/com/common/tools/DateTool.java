...
public class DateTool {
	 ...
	 * @throws Exception 
	 */
	public static Date addDateTime(Date dateSource, String unit, int number) throws Exception {
		if (dateSource == null) {
			throw new Exception();
		};
	 ...
	 * @throws Exception 
	 */
	public static String addToday(int days) throws Exception {
	 ...
	 * 计算两个日期相差的月数
	 * 说明：忽略天数，只看年与月份
	 * 计算方法：先获取相差多少年a ，(a+1)*12 - (前一年的月份数)- ( 12 - 后一年的月份数 ) 
	 * @param dt1  
	 * 				-前一个日期（早）
	 * @param dt2
	 * 				-后一个日期
	 * @return
	 */
	public static int getMonthsBetween(Date dt1, Date dt2){
		// 获取合同开始日期的天数
	    ...
	    c1.setTime(dt1);
        c2.setTime(dt2);
        int Y1 = c2.get(Calendar.YEAR) - c1.get(Calendar.YEAR);
    	// 为了处理 2016-04-01 -- 2018-03-31 这种特殊零界点会少一个月情况
        if( c1.get(Calendar.DATE) != 1){
            result =  (Y1+1) * 12 - c1.get(Calendar.MONTH) - (12 - c2.get(Calendar.MONTH)) ;      
        }else{
            result =  (Y1+1) * 12 - c1.get(Calendar.MONTH) - (12 - c2.get(Calendar.MONTH)) +1 ;      
        }
		return result ;		
	}
	...
	public static void main(String[] args) throws ParseException {
		...
		Date dt1 = DateTool.toDate("2015-10-02");
		Date dt2 = DateTool.toDate("2016-10-01");
				
		System.out.println((double)DateTool.getMonthsBetween(dt1, dt2));	
